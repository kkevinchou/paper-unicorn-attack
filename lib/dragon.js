...
Dragon.prototype.fireBall = function() {
	...
	if (self.isOnCooldown || !self.active) {
		return null;
	}
    ...
    var fireball = new fireballModule.FireBall(self.engine, self.x+fireBallXSpeed*250, self.y+fireBallYSpeed*250, fireBallXSpeed, fireBallYSpeed);

    self.engine.sendOneTimeEvent('audio', {filename: 'dragon_roar.wav'});
...
Dragon.prototype.takeDamage = function() {
    ...
    self.engine.sendOneTimeEvent('slash', {x: self.x, y: self.y});
...
Dragon.prototype.die = function() {
	...
	self.engine.sendOneTimeEvent('audio', { filename: 'dragon_death.wav' });
